      ...
      <form onSubmit={FormProps.handleSubmit} className="flex flex-col space-y-4">
        ...
        <div className="flex flex-col">
          ...
          <textarea
            ...
            onChange={(e) =>
              FormProps.setPost({ ...FormProps.post, description: e.target.value })
            }
            className="border border-gray-300 p-2 rounded-md"
          />
        </div>
        <div className="flex flex-col">
          <label htmlFor="isImportant" className="text-lg font-semibold">
            Important
          </label>
          <input
            type="checkbox"
            ...
            onChange={(e) =>
              FormProps.setPost({ ...FormProps.post, isImportant: e.target.checked })
            }
            className="border border-gray-300 p-2 rounded-md"
          />
        </div>
        <button
          type="submit"
          disabled={FormProps.submitting || FormProps.post.title.trim() === ""}
          className={cn(["bg-blue-500 text-white p-2 rounded-md transition-opacity duration-200", FormProps.post.title.trim() === "" ? "opacity-70" : ""])}
        >
          {FormProps.submitting ? `${FormProps.type}...` : FormProps.type}
        </button>
      </form>
      <Link href="/">
